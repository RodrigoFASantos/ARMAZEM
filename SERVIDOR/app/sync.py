...
def sync_tipos():
    """Retorna todos os tipos para sincronização"""
    try:
        conn = get_connection()
        cur = conn.cursor()
...
def sync_familias():
    """Retorna todas as famílias para sincronização"""
    try:
        conn = get_connection()
        cur = conn.cursor()
...
def sync_armazens():
    """Retorna todos os armazéns para sincronização"""
    try:
        conn = get_connection()
        cur = conn.cursor()
        
        cur.execute("SELECT ID_armazem, Descricao, Localizacao FROM Armazem")
        rows = cur.fetchall()
...
def sync_artigos():
    """Retorna todos os artigos para sincronização"""
    try:
        conn = get_connection()
        cur = conn.cursor()
...
def sync_equipamentos():
    """Retorna todos os equipamentos para sincronização"""
    try:
        conn = get_connection()
        cur = conn.cursor()
...
def sync_movimentos():
    """Retorna todos os movimentos para sincronização"""
    try:
        conn = get_connection()
        cur = conn.cursor()
...
def sync_utilizadores():
    """Retorna todos os utilizadores para sincronização"""
    try:
        conn = get_connection()
        cur = conn.cursor()
